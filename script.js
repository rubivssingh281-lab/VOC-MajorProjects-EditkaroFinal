document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!prefersReduced) {
        ...
        window.__lenisInstance = lenis; (function raf(time) { lenis.raf(time); requestAnimationFrame(raf); })();
    ...
    document.querySelectorAll('.sticky-section').forEach((section, idx) => {
        ...
        pillars.forEach((el, i) => {
            ...
            const globalIdx = idx * 3 + i;
            animatePillar(tl, el, globalIdx, t);
        });
    });

    /* ---------- Hero card ---------- */
    (function heroCardExpand() {
        document.querySelectorAll('.hero-card[data-expand]').forEach(card => {
            const key = card.dataset.expand;
            const overlay = document.getElementById(`expand-${key}`);
            if (!overlay) return;

            const inner = overlay.querySelector('.card-expand-inner');
            const blocks = overlay.querySelectorAll('.ce-block');
            const closeBtn = overlay.querySelector('.card-expand-close');
            const scrollArea = overlay.querySelector('.card-expand-scroll');
            const destEyebrow = overlay.querySelector('.ce-eyebrow');
            const destTitle = overlay.querySelector('.ce-title');
            const srcTag = card.querySelector('.tag');
            const srcTitle = card.querySelector('.card-title');
            let isOpen = false;
            let liveClones = [];

            function measureDestRects() {
                const prevInline = overlay.style.cssText;
                gsap.set(overlay, { top: 0, left: 0, width: '100vw', height: '100vh', borderRadius: 0 });
                const eyebrowRect = destEyebrow.getBoundingClientRect();
                const titleRect = destTitle.getBoundingClientRect();
                const eyebrowStyle = getComputedStyle(destEyebrow);
                const titleStyle = getComputedStyle(destTitle);
                const result = {
                    eyebrow: { rect: eyebrowRect, fontSize: eyebrowStyle.fontSize, letterSpacing: eyebrowStyle.letterSpacing, color: eyebrowStyle.color, lineHeight: eyebrowStyle.lineHeight },
                    title: { rect: titleRect, fontSize: titleStyle.fontSize, lineHeight: titleStyle.lineHeight, color: titleStyle.color, letterSpacing: titleStyle.letterSpacing }
                };
                overlay.style.cssText = prevInline;
                return result;
            }

            function makeClone(el, rect, styleInfo) {
                const clone = el.cloneNode(true);
                clone.style.position = 'fixed';
                clone.style.margin = '0';
                clone.style.zIndex = '9700';
                clone.style.pointerEvents = 'none';
                clone.style.top = rect.top + 'px';
                clone.style.left = rect.left + 'px';
                clone.style.width = rect.width + 'px';
                clone.style.fontSize = styleInfo.fontSize;
                clone.style.lineHeight = styleInfo.lineHeight;
                clone.style.color = styleInfo.color;
                clone.style.letterSpacing = styleInfo.letterSpacing;
                document.body.appendChild(clone);
                return clone;
            }

            function cleanupClones() {
                liveClones.forEach(c => c.remove());
                liveClones = [];
            }
            ...
            function openCard() {
                ...
                const tagRect = srcTag ? srcTag.getBoundingClientRect() : null;
                const titleRect = srcTitle ? srcTitle.getBoundingClientRect() : null;
                const tagStyle = srcTag ? getComputedStyle(srcTag) : null;
                const titleStyle = srcTitle ? getComputedStyle(srcTitle) : null;

                gsap.set(overlay, {
                    position: 'fixed', top: rect.top, left: rect.left,
                    width: rect.width, height: rect.height,
                    borderRadius: 18, opacity: 1, visibility: 'visible', overflow: 'hidden'
                });
                overlay.classList.add('is-open');
                document.body.style.overflow = 'hidden';
                if (window.__lenisInstance) window.__lenisInstance.stop();

                gsap.set([destEyebrow, destTitle], { opacity: 0 });

                const dest = (srcTag && srcTitle) ? measureDestRects() : null;
                let cloneTag, cloneTitle;
                if (dest) {
                    cloneTag = makeClone(srcTag, tagRect, { fontSize: tagStyle.fontSize, lineHeight: tagStyle.lineHeight, color: tagStyle.color, letterSpacing: tagStyle.letterSpacing });
                    cloneTitle = makeClone(srcTitle, titleRect, { fontSize: titleStyle.fontSize, lineHeight: titleStyle.lineHeight, color: titleStyle.color, letterSpacing: titleStyle.letterSpacing });
                    liveClones = [cloneTag, cloneTitle];
                    gsap.set(srcTag, { opacity: 0 });
                    gsap.set(srcTitle, { opacity: 0 });
                }

                gsap.to(overlay, {
                    top: 0, left: 0, width: '100vw', height: '100vh',
                    borderRadius: 0, duration: .85, ease: 'power4.inOut',
                    onComplete: () => gsap.set(overlay, { position: 'fixed', inset: 0, width: '', height: '' })
                });

                if (dest) {
                    gsap.to(cloneTag, {
                        top: dest.eyebrow.rect.top, left: dest.eyebrow.rect.left,
                        width: dest.eyebrow.rect.width, fontSize: dest.eyebrow.fontSize,
                        letterSpacing: dest.eyebrow.letterSpacing,
                        duration: .85, ease: 'power4.inOut'
                    });
                    gsap.to(cloneTitle, {
                        top: dest.title.rect.top, left: dest.title.rect.left,
                        width: dest.title.rect.width, fontSize: dest.title.fontSize,
                        lineHeight: dest.title.lineHeight,
                        duration: .85, ease: 'power4.inOut',
                        onComplete: () => {
                            gsap.set(destEyebrow, { opacity: 1 });
                            gsap.set(destTitle, { opacity: 1 });
                            gsap.to([cloneTag, cloneTitle], {
                                opacity: 0, duration: .18, ease: 'power1.out',
                                onComplete: cleanupClones
                            });
                        }
                    });
                }

                gsap.to(inner, { opacity: 1, duration: .5, delay: .35, ease: 'power2.out' });
                gsap.fromTo(blocks,
                    { opacity: 0, y: 24 },
                    { opacity: 1, y: 0, duration: .6, stagger: .1, delay: .5, ease: 'power3.out' }
                );
            }
            ...
            function closeCard() {
                ...
                gsap.to(inner, { opacity: 0, duration: .25, ease: 'power2.in' });

                let cloneTag, cloneTitle;
                if (srcTag && srcTitle && tagRect && titleRect) {
                    const eyebrowStyle = getComputedStyle(destEyebrow);
                    const titleStyle = getComputedStyle(destTitle);
                    const eyebrowRect = destEyebrow.getBoundingClientRect();
                    const titleRectNow = destTitle.getBoundingClientRect();

                    cloneTag = makeClone(destEyebrow, eyebrowRect, { fontSize: eyebrowStyle.fontSize, lineHeight: eyebrowStyle.lineHeight, color: eyebrowStyle.color, letterSpacing: eyebrowStyle.letterSpacing });
                    cloneTitle = makeClone(destTitle, titleRectNow, { fontSize: titleStyle.fontSize, lineHeight: titleStyle.lineHeight, color: titleStyle.color, letterSpacing: titleStyle.letterSpacing });
                    liveClones = [cloneTag, cloneTitle];

                    gsap.set([destEyebrow, destTitle], { opacity: 0 });
                    gsap.set([srcTag, srcTitle], { opacity: 0 });

                    const srcTagStyle = getComputedStyle(srcTag);
                    const srcTitleStyle = getComputedStyle(srcTitle);

                    gsap.to(cloneTag, {
                        top: tagRect.top, left: tagRect.left, width: tagRect.width,
                        fontSize: srcTagStyle.fontSize, letterSpacing: srcTagStyle.letterSpacing,
                        duration: .7, ease: 'power4.inOut'
                    });
                    gsap.to(cloneTitle, {
                        top: titleRect.top, left: titleRect.left, width: titleRect.width,
                        fontSize: srcTitleStyle.fontSize, lineHeight: srcTitleStyle.lineHeight,
                        duration: .7, ease: 'power4.inOut',
                        onComplete: () => {
                            gsap.set(srcTag, { opacity: 1 });
                            gsap.set(srcTitle, { opacity: 1 });
                            gsap.to([cloneTag, cloneTitle], {
                                opacity: 0, duration: .18, ease: 'power1.out',
                                onComplete: cleanupClones
                            });
                ...
                gsap.to(overlay, {
                    ...
                    width: rect.width, height: rect.height,
                    borderRadius: 18, duration: .7, ease: 'power4.inOut',
                    onComplete: () => {
                        overlay.classList.remove('is-open');
                        gsap.set(overlay, { opacity: 0, visibility: 'hidden' });
                        gsap.set(blocks, { opacity: 0, y: 24 });
                        gsap.set([destEyebrow, destTitle], { opacity: 1 });
            ...
            scrollArea.addEventListener('wheel', e => { if (!isOpen) return; e.stopPropagation(); }, { passive: true });
        });
    })();
    ...
    if (!isTouch && !prefersReduced) {
        document.querySelectorAll('.pillar-link').forEach(link => {
            const video = link.querySelector('.pillar-video-bg');
            link.addEventListener('mousemove', e => {
                const r = link.getBoundingClientRect();
                const px = (e.clientX - r.left) / r.width - .5;
                const py = (e.clientY - r.top) / r.height - .5;
                gsap.to(video, { x: px * 14, y: py * 14, duration: .5, ease: 'power2.out' });
            });
            link.addEventListener('mouseleave', () => {
                gsap.to(video, { x: 0, y: 0, duration: .6, ease: 'power3.out' });
            });
        });
    }
    ...
    /* ---------- pillar card ---------- */
    document.querySelectorAll('.pillar-link').forEach(card => {
        const pillar = card.closest('.pillar');
        const more = card.querySelector('.pillar-more');
        const cta = card.querySelector('.pillar-cta');
        let expanded = false;
        let animating = false;
        ...
        function toggleExpand() {
            ...
            pillar.classList.toggle('is-expanded', expanded);
            card.setAttribute('aria-expanded', String(expanded));
            if (cta) cta.textContent = expanded ? 'Show less' : 'Read more';

            if (expanded) {
                gsap.set(more, { height: 'auto', opacity: 0 });
                const fullHeight = more.offsetHeight;
                gsap.set(more, { height: 0 }); 
                ...
                const currentHeight = more.offsetHeight;
                gsap.set(more, { height: currentHeight });
                void more.offsetHeight;
            ...
            if (lenis) {
                lenis.stop();
                clearTimeout(pillar._resumeTimer);
                pillar._resumeTimer = setTimeout(() => lenis.start(), 750);
            }
        }
    ...
    (function () {
        ...
        const scrollArea = document.getElementById('catPanelScroll');

        if (!overlay || !panel || !inner || !scrollArea) return;

        let isOpen = false;

        function buildContent(row) {
            const name = row.querySelector('.cat-name')?.childNodes[0]?.textContent?.trim() || '';
            const count = row.dataset.count || row.querySelector('.cat-count')?.textContent || '';
            const desc = row.dataset.desc || 'Details for this category are coming soon.';
            const heroImg = row.dataset.img || '';
            const gallery = (row.dataset.gallery || '').split(',').map(s => s.trim()).filter(Boolean);

            inner.innerHTML = `
      <button class="cat-panel-close" id="catPanelClose" aria-label="Close category details">
        <span></span><span></span>
      </button>
      <div class="cp-eyebrow">Category — ${count} works</div>
      <h2 class="cp-title display">${name}</h2>
      <div class="cp-hero-img"><img src="${heroImg}" alt="${name}" /></div>
      <p class="cp-desc">${desc}</p>
      <div class="cp-gallery">
        ${gallery.map(src => `<img src="${src}" alt="${name}" />`).join('')}
      </div>
    `;
            inner.querySelector('#catPanelClose').addEventListener('click', close);
        }
        ...
        function open(row) {
            ...
            gsap.to(panel, { height: 'calc(100vh - 84px)', duration: .8, ease: 'power4.inOut' });
            gsap.to(inner, { opacity: 1, duration: .5, delay: .3, ease: 'power2.out' });

            gsap.set('.cp-gallery img', { opacity: 0, y: 20 });
            gsap.to('.cp-gallery img', { opacity: 1, y: 0, duration: .6, stagger: .1, ease: 'power3.out', delay: .45 });
        }
        ...
        function close() {
            ...
            gsap.to(inner, { opacity: 0, duration: .2, ease: 'power2.in' });
            gsap.to(panel, {
                height: 0, duration: .6, ease: 'power4.inOut',
                onComplete: () => {
                    document.body.style.overflow = '';
                    if (window.__lenisInstance) window.__lenisInstance.start();
                }
    ...
    if (!isTouch) {
        ...
        document.querySelectorAll('.project-row').forEach(row => {
            row.addEventListener('mouseenter', () => {
                ...
                gsap.to(vf, { width: 370, height: 220, duration: .45, ease: 'power3.out' });
            });
            row.addEventListener('mouseleave', () => {
    ...
    /* ---------- Tilt effect for project rows on hover ---------- */
    if (!isTouch && !prefersReduced) {
        document.querySelectorAll('.project-row').forEach(row => {
            row.addEventListener('mousemove', e => {
                const r = row.getBoundingClientRect();
